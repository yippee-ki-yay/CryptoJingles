...
import {
  CryptoJinglesAddress, JingleAddress, MarketplaceAddress, SampleAddress, API_URL, CURRENT_NETWORK,
...
import { nameOfNetwork } from '../services/generalService';
...
const getWeb3 = async (dispatch) => {
  if (typeof web3 === 'undefined') return dispatch(initAppWithoutMM());
  ...
  if (CURRENT_NETWORK !== nameOfNetwork(network)) networkError = true;

  if (!address) return dispatch(initAppWithLockedMM(network, networkError));

  const canLikeResponse = await axios(`${API_URL}/jingle/can-like/${address.toLowerCase()}`);
  const { canLike } = canLikeResponse.data;
  dispatch(initAppWithMM(address.toLowerCase(), canLike, network, networkError));
};
