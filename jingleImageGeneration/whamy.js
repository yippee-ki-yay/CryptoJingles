...
global.Whammy = (function () {
  ...
  function parseWebP(riff) {
    ...
    let width; let horizontal_scale; let height; let vertical_scale; let
      tmp;
  ...
  function parseRIFF(string) {
    ...
    while (offset < string.length) {
      ...
      const len = parseInt(string.substr(offset + 4, 4).split('').map((i) => {
  ...
  function doubleToString(num) {
    return [].slice.call(
      ...
      .map((e) => // since it's a regular array, we can now use map
        String.fromCharCode(e), // encode all the bytes individually
      )
  ...
  WhammyVideo.prototype.compile = function (outputAsArray) {
    return new toWebM(this.frames.map((frame) => {
